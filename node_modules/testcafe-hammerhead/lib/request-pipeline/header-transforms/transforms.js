...
function generateSyncCookie(ctx, parsedServerCookies) {
    ...
    let syncWithClientCookies = parsedServerCookies
        ...
        return (0, cookie_1.formatSyncCookie)(cookie);
    });
    if (ctx.parsedClientSyncCookie) {
        const outdatedSyncCookies = ctx.parsedClientSyncCookie.actual.filter(clientCookie => {
            ...
            for (const serverCookie of parsedServerCookies) {
                if ((0, cookie_1.isOutdatedSyncCookie)(clientCookie, serverCookie))
                    return true;
            }
            return false;
        });
...
function resolveAndGetProxyUrl(url, ctx) {
    ...
    const { host } = (0, url_1.parse)(url);
    let isCrossDomain = false;
    if (!host)
        url = (0, url_1.resolve)(ctx.dest.url, url);
...
function processSetCookieHeader(src, ctx) {
    const parsedCookies = src && !(0, same_origin_policy_1.shouldOmitCredentials)(ctx) ? ctx.session.cookies.setByServer(ctx.dest.url, src) : [];
...
exports.forcedRequestTransforms = {
    [builtin_header_names_1.default.cookie]: (_src, ctx) => (0, same_origin_policy_1.shouldOmitCredentials)(ctx) ? void 0 : ctx.session.cookies.getHeader(ctx.dest) || void 0,
};
// Response headers
